...
import uc.seng301.wordleapp.lab5.dictionary.DictionaryQuery;
...
public class GetGuessesFeature {
    private List<String> words;
    private String known;
    private List<String> allWords = new ArrayList<>(Arrays.asList("which", "there", "their", "about", "would", "these",            "other", "words", "could", "write"));
...
//        Assertions.assertEquals(words, allWords.stream().filter(s -> s.matches(known)).collect(Collectors.toList()));
    ...
    public void no_words_are_returned() {
        words = mockDictionaryQuery.guessWord(known).getWords();
        Assertions.assertTrue(words.size() == 0);
//        Assertions.assertEquals(words, allWords.stream().filter(s -> s.matches(known)).collect(Collectors.toList()));
    }
